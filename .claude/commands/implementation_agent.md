...
1. **File operations**: Use Read, Write, Edit, and MultiEdit tools for Python code changes.
   - Read existing Python files to understand current structure
   - Write new Python files according to the plan specifications
   - Edit existing Python files to add new functionality
   - Use MultiEdit for complex changes across multiple sections
   - Follow Python naming conventions (snake_case) and PEP 8 standards
...
3. **Command execution**: Use Bash tool for running Python commands and tests.
   - Install dependencies using uv (preferred), poetry, or conda
   - Run pytest test suites and validation commands
   - Execute Python scripts and applications with uv run
   - Run linting (ruff), formatting (black), and type checking (mypy)
   - Manage virtual environments with uv venv
...
### Deliverables
**Files Created**:
- `file1.py` - [Description and purpose]
- `test_file2.py` - [Test file description]

**Files Modified**:
- `existing_file.py` - [What was changed and why]
